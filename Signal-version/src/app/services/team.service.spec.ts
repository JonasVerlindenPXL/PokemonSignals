...
describe('TeamService', () => {
  ...
  //tried without fakeAsync or done
  it('should add a Pokemon to the team with delay used fakeAsync', () => {

    expect(service.pokemons.length).toBe(0);
    service.addToTeam(pokemon);
    expect(service.pokemons.length).toBe(1);
    expect(service.pokemons()[0]).toEqual(pokemon);
